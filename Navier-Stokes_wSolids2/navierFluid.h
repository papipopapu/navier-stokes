...
void paintPixels(grid &s, grid &solidMap, std::vector<sf::Uint8> &pixels, float r, float g, float b, int a, float max, float min, bool gradient, bool paintTwo, grid &optional); // to's end's not included
...
void getW(grid &u, grid &v, grid &w);
...
class fluid
{
    ...
    grid u, v, u0, v0, s, s0, p, solidMap, w;
...
class simulation
{
    ...
    float playSpeed = 0.2;
    bool gradient = true, normalize = true;
    
    uint8_t a = 255; // colors
    uint8_t draw = 0, radius = 3;// 0 for s, 1 for u, 2 for v, 3 for p
    uint8_t fps = 33;
    float drawMin, drawMax;
    // maxs: u ~ 400, s ~ 200, p ~ 1000
