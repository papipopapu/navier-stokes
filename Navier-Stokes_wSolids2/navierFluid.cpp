...
void navier::getW(grid &u, grid &v, grid &w)
{
    for (int i=1; i<u.getNi()-1; i++)
    {
        for (int j=1; j<u.getNj(); j++)
        {
            w(i, j) = fabs( 0.5 * (u(i, j+1) - u(i, j-1) + v(i-1, j)- v(i+1, j)) );
        }
    }
}
...
bool static compare(float a, float b)
{
...
void navier::simulation::start()
{
    ...
    int i, j, ip, jp;
    float t = 0, factorI, factorJ, x , y, x0, y0, min, max;
    ...
    while(window.isOpen())
    {
        ...
        switch(draw)
        {
            ...
            if (normalize)
            {
                ...
                min = drawMin; max = drawMax;
            }
            paintPixels(Fluid.s, Fluid.solidMap, pixels, r, g, b, a, max, min, gradient, false, Fluid.u); break;
            case 1: 
            if (normalize)
            {
            ...
            paintPixels(Fluid.u, Fluid.solidMap, pixels, r, g, b, a, max, min, gradient, false, Fluid.u); break; 
            case 2:    
            if (normalize)
            {
            ...
            paintPixels(Fluid.v, Fluid.solidMap, pixels, r, g, b, a, max, min,gradient, false, Fluid.u); break;
            case 3: 
            if (normalize)
            {
            ...
            paintPixels(Fluid.p, Fluid.solidMap, pixels, r, g, b, a, max, min, gradient, false, Fluid.u); break;
            case 4:
            if (normalize)
            {
                minMax = std::minmax_element(Fluid.u.getV().begin(), Fluid.u.getV().end());
            ...
            paintPixels(Fluid.u, Fluid.solidMap, pixels, r, g, b, a, max, min, gradient, true, Fluid.v); break;
            case 5:
            if (normalize)
            {
                minMax = std::minmax_element(Fluid.w.getV().begin(), Fluid.w.getV().end());
                min = *minMax.first; max = *minMax.second;
            } else {
                min = drawMin; max = drawMax;
            }
            paintPixels(Fluid.w, Fluid.solidMap, pixels, r, g, b, a, max, min, gradient, false, Fluid.v); break;
...
void navier::paintPixels(grid &s, grid &solidMap, std::vector<sf::Uint8> &pixels, float  r, float g, float b, int a, float max, float min, bool gradient, bool paintTwo, grid &optional) // fuck boundaries
{
    int k=0, Nj = s.getNj(), Ni = s.getNi();
    float val, norm, p;
    int normalized, region;
    sf::Uint8 x;
    ...
    for(int i=1; i<Ni-1; i++)
    {
        for (int j=1; j<Nj-1; j++)
        {
            ...
            {   
                if (gradient)
                {
                    norm = paintTwo ?  (std::sqrt(std::pow(s(i,j),2)+std::pow(optional(i,j),2)) - min)/(max - min) :   (fabs(s(i,j)) - min)/(max - min);
                    normalized = int(norm * 256 * 3);
                    ...
                    switch (region)
                    {
                    case 0: pixels[k] = 0;                  pixels[k+1] =  0;           pixels[k+2] = x;   break;
                    case 1: pixels[k] = 0;                  pixels[k+1] =  x;           pixels[k+2] = 255;   break;
                    case 2: pixels[k] = 0;                  pixels[k+1] =  255;           pixels[k+2] = 255-x;   break;        
                    }
                    pixels[k+3] = a;
       
                } else
                {
                norm = paintTwo ?  (s(i,j)+optional(i,j) - min)/(max - min) :   (s(i,j) - min)/(max - min);
                val = norm * 255;
